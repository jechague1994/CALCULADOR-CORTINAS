...
def recomendar_motor(tipo, m2, usos):
    ...
    if usos <= 7:
        ...
        if m2 <= 18: recomendaciones.append("Motor Parallel 600")
        elif m2 <= 28: recomendaciones.append("Motor Parallel 800")
        elif m2 <= 32: recomendaciones.append("Motor Parallel 1000")
        elif m2 <= 42: recomendaciones.append("Motor Parallel 1500")
...
def calcular_estructura_acero(ancho_m, alto_m):
    # Definimos la escala de ejes y guías
    escalas = [
        {"max_ancho": 5, "eje": "Eje redondo 101mm", "guia": "Guías 60x50mm"},
        {"max_ancho": 6, "eje": "Eje 127mm", "guia": "Guías 80x60mm"},
        {"max_ancho": 7, "eje": "Eje 152mm", "guia": "Guías 80x60mm"},
        {"max_ancho": 8, "eje": "Eje 200mm", "guia": "Guías 100x60mm"},
        {"max_ancho": 99, "eje": "Eje 250mm", "guia": "Guías 150x60mm"}
    ]
    
    # Encontrar el índice que corresponde por ancho
    indice = 0
    for i, esc in enumerate(escalas):
        if ancho_m < esc["max_ancho"]:
            indice = i
            break
            
    # REGLA SOLICITADA: Si el alto > 3m, saltar a la próxima medida de eje
    if alto_m > 3.0 and indice < len(escalas) - 1:
        indice += 1
        
    res = escalas[indice]
    return res["eje"], res["guia"]
...
with st.sidebar:
    ...
    ancho_m = st.number_input("Ancho (metros)", min_value=0.1, value=4.0, step=0.1)
    alto_m = st.number_input("Alto (metros)", min_value=0.1, value=3.0, step=0.1)
...
excedente_m = (0.30 if fam == "alu55" else 0.40) if agregar_enrollado else 0
    ...
    if os.path.exists(archivo_precios):
        try:
            ...
            if 'Producto' in df.columns and 'Precio' in df.columns:
                ...
                cols = ['Producto', 'Precio ARS ($)']
                if 'Unidad' in df.columns: cols.append('Unidad')
                st.dataframe(df_vis[cols], use_container_width=True, hide_index=True)
